...
Issues 01–19 的工程实现与确定性验收已经完成。Issue 20 仍有以下外部发布动作：
...
这些任务需要凭据、Docker、远端仓库或人工视觉确认，不应由本地确定性结果冒充完成。
...
建议先完成 Issue 21 的持久化契约，再并行推进 22 和 23；24 为恢复证据底座，25 可在恢复语义稳定后接入，最后由 26 统一做长期运行与安全验收。
